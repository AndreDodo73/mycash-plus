    ...
    >
      <div className="mb-space-8 flex justify-end">
        <button
          type="button"
          onClick={onClose}
          className="flex size-space-56 items-center justify-center rounded-shape-100 transition-colors hover:bg-neutral-100"
      ...
      <nav className="flex flex-col gap-space-8" aria-label="Principal">
        {APP_ROUTES.map((route) => (
          <NavLink
            ...
            className={({ isActive }) =>
              [
                "flex min-h-space-56 items-center gap-space-12 rounded-shape-100 px-space-16 py-space-12 text-label-large font-semibold transition-colors",
                isActive
                  ? "bg-secondary text-surface [&_img]:brightness-0 [&_img]:invert"
                  : "bg-transparent text-neutral-600 hover:bg-neutral-100 hover:text-neutral-1100",
              ].join(" ")
            }
          >
            {({ isActive }) => (
